...
export class GLSLHighlighter {
    ...
    /**
     * Highlight GLSL code and return HTML
     * @param {string} code - Raw GLSL code
     * @returns {string} - HTML with syntax highlighting
     */
    highlight(code) {
        // THIS SECTION GENERATED BY Claude - Fixed highlighting to prevent tag conflicts
        const tokens = this.tokenize(code);
        return tokens.map(token => {
            if (token.type === 'text') {
                return this.escapeHtml(token.value);
            } else {
                return `<span style="color: ${this.colors[token.type]}">${this.escapeHtml(token.value)}</span>`;
            }
        }).join('');
    }
    ...
    createOverlay(textarea) {
        ...
        Object.assign(overlay.style, {
            ...
            overflow: 'hidden',
            pointerEvents: 'none',
            color: '#cccccc', // THIS SECTION GENERATED BY Claude - Default text color for unhighlighted text
