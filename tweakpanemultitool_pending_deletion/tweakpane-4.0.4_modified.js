...
const ICON_ID_TO_INNER_HTML_MAP = {
    ...
    // THIS SECTION GENERATED BY Claude - Adding icons for 3D and 4D pads
    p3dpad: '<path d="M8 4v8"/><path d="M4 8h8"/><circle cx="12" cy="12" r="1.2"/>',
    p4dpad: '<path d="M8 4v8"/><path d="M4 8h8"/><circle cx="12" cy="12" r="1.2"/>',
};
...
class TextView {
    constructor(doc, config) {
        ...
        // Check if multiline is enabled from props - with safe property access
        let isMultiline = false;
        let lines = 3;
        let wordwrap = true; // Default to true (word wrapping enabled)
        
        try {
            isMultiline = this.props_.get('multiline') || false;
        } catch (e) {
            // multiline property doesn't exist, default to false
            isMultiline = false;
        }
        
        try {
            lines = this.props_.get('lines') || 3;
        } catch (e) {
            // lines property doesn't exist, default to 3
            lines = 3;
        }
        
        try {
            wordwrap = this.props_.get('wordwrap');
            if (wordwrap === undefined) wordwrap = true; // Default to true if not specified
        } catch (e) {
            // wordwrap property doesn't exist, default to true
            wordwrap = true;
        }
        
        let inputElem;
        if (isMultiline) {
            inputElem = doc.createElement('textarea');
            inputElem.classList.add(cn$i('ta')); // Different class for textarea
            inputElem.rows = lines;
            inputElem.style.resize = 'none';
            inputElem.style.overflowY = 'auto';
            inputElem.spellcheck = false; // THIS SECTION GENERATED BY Claude - Disable spellcheck
            
            // Handle word wrapping - if disabled, prevent wrapping and add horizontal scroll
            if (!wordwrap) {
                inputElem.style.whiteSpace = 'pre';  // THIS SECTION GENERATED BY Claude - preserve line breaks but no word wrap
                inputElem.style.overflowX = 'auto';
                inputElem.style.wordWrap = 'normal';
            } else {
                inputElem.style.whiteSpace = 'pre-wrap';
                inputElem.style.overflowX = 'hidden';
                inputElem.style.wordWrap = 'break-word';
            }
        } else {
        ...
        // END SECTION GENERATED BY CLAUDE
...
class TextController {
    constructor(doc, config) {
        ...
        // THIS SECTION GENERATED BY CLAUDE
        // Support both 'change' and 'input' events for textarea
        this.view.inputElement.addEventListener('change', this.onInputChange_);
        this.view.inputElement.addEventListener('input', this.onInputChange_);
        // END SECTION GENERATED BY CLAUDE
    }
    onInputChange_(e) {
...
// THIS SECTION GENERATED BY Claude - Point3dView with button (reusing Point2d CSS classes)
const cn$p2d_reuse = ClassName('p2d'); // Reuse Point2d CSS classes for proper button styling
class Point3dView {
    constructor(doc, config) {
        // Create main container
        this.element = doc.createElement('div');
        this.element.classList.add(cn$p2d_reuse()); // Use p2d CSS for layout
        config.viewProps.bindClassModifiers(this.element);
        
        // Create header with button and text inputs
        const headElem = doc.createElement('div');
        headElem.classList.add(cn$p2d_reuse('h')); // Use p2d header CSS
        this.element.appendChild(headElem);
        
        // Create the "+" button using Point2d CSS classes for proper sizing
        const buttonElem = doc.createElement('button');
        buttonElem.classList.add(cn$p2d_reuse('b')); // Use p2d button CSS
        buttonElem.appendChild(createSvgIconElement(doc, 'p3dpad'));
        config.viewProps.bindDisabled(buttonElem);
        headElem.appendChild(buttonElem);
        this.buttonElement = buttonElem;
        ...
        const textElem = doc.createElement('div');
        textElem.classList.add(cn$p2d_reuse('t')); // Use p2d text CSS
        headElem.appendChild(textElem);
        this.textElement = textElem;
    }
}

// THIS SECTION GENERATED BY Claude - Point3dController that combines button view with text inputs
// THIS SECTION GENERATED BY Claude - Point3dController with proper popup
class Point3dController {
    constructor(doc, config) {
        ...
        // Flags to prevent circular updates - THIS SECTION GENERATED BY Claude
        this.updatingFromMain_ = false;
        this.updatingFromSlider_ = false;
        
        // Create foldable state for popup
        this.foldable_ = Foldable.create(false); // Start collapsed
        ...
        // Create popup content container with proper Tweakpane styling
        this.sliderContainer_ = doc.createElement('div');
        this.sliderContainer_.style.padding = '8px';
        this.sliderContainer_.style.backgroundColor = '#2f3542';
        this.sliderContainer_.style.border = '1px solid #404754';
        this.sliderContainer_.style.borderRadius = '2px';
        this.sliderContainer_.style.width = '100%'; // Match parent width
        this.sliderContainer_.style.boxSizing = 'border-box';
        
        // Position popup to align with right edge of control
        this.popC_.view.element.style.position = 'absolute';
        this.popC_.view.element.style.right = '0';
        this.popC_.view.element.style.top = '100%';
        this.popC_.view.element.style.zIndex = '1000';
        this.popC_.view.element.style.width = '100%'; // Match parent width
        
        // Create 3 slider controls using proper Tweakpane controllers
        this.sliderControllers_ = [];
        const labels = ['X', 'Y', 'Z'];
        const components = this.value.rawValue.getComponents();
        
        labels.forEach((label, index) => {
            // Create wrapper for each slider with label
            ...
            labelElem.style.minWidth = '20px';
            labelElem.style.fontSize = '11px';
            labelElem.style.color = '#ccc';
            labelElem.style.fontFamily = 'inherit';
            sliderWrapper.appendChild(labelElem);
            
            // Create individual value for this component
            const componentValue = createValue(components[index]);
            
            // Get min/max from axis constraints or use defaults - THIS SECTION GENERATED BY Claude
            const axis = config.axes[index];
            const constraint = axis ? axis.constraint : null;
            let minVal = -2, maxVal = 2; // defaults
            
            // Extract DefiniteRangeConstraint from composite constraint - THIS SECTION GENERATED BY Claude
            const drc = constraint && findConstraint(constraint, DefiniteRangeConstraint);
            if (drc) {
                minVal = drc.values.value('min').rawValue;
                maxVal = drc.values.value('max').rawValue;
            }
            
            // Create proper Tweakpane slider controller (slider only, no text input)
            const sliderController = new SliderController(doc, {
                props: new ValueMap({
                    keyScale: createValue(1),
                    max: createValue(maxVal),
                    min: createValue(minVal)
                }),
                value: componentValue,
                viewProps: this.viewProps
            });
            
            // Update main value when slider changes - THIS SECTION GENERATED BY Claude
            componentValue.emitter.on('change', () => {
                if (this.updatingFromMain_) return; // Prevent circular updates
                this.updatingFromSlider_ = true;
                const newComponents = [...this.value.rawValue.getComponents()];
                newComponents[index] = componentValue.rawValue;
                const newValue = new Point3d(newComponents[0], newComponents[1], newComponents[2]);
                this.value.rawValue = newValue;
                this.updatingFromSlider_ = false;
            });
            
            // Update slider when main value changes - THIS SECTION GENERATED BY Claude
            this.value.emitter.on('change', () => {
                if (this.updatingFromSlider_) return; // Prevent circular updates
                this.updatingFromMain_ = true;
                const currentComponents = this.value.rawValue.getComponents();
                componentValue.rawValue = currentComponents[index];
                this.updatingFromMain_ = false;
            });
            
            // Add blur event for popup management
            sliderController.view.element.addEventListener('blur', this.onPopupChildBlur_);
            
            // Add slider to wrapper
            const sliderDiv = doc.createElement('div');
            sliderDiv.style.flex = '1';
            sliderDiv.appendChild(sliderController.view.element);
            sliderWrapper.appendChild(sliderDiv);
            
            this.sliderContainer_.appendChild(sliderWrapper);
            this.sliderControllers_.push(sliderController);
        });
        
        // Create the text controller for X,Y,Z inputs
        this.textC_ = new PointNdTextController(doc, {
        ...
        this.view = new Point3dView(doc, {
            viewProps: this.viewProps,
        });
        
        // Add the text inputs to the view
        this.view.textElement.appendChild(this.textC_.view.element);
        
        // Setup popup
        this.view.element.appendChild(this.popC_.view.element);
        this.popC_.view.element.appendChild(this.sliderContainer_);
        
        // Connect foldable state to popup visibility
        connectValues({
            primary: this.foldable_.value('expanded'),
            secondary: this.popC_.shows,
            forward: (p) => p,
        ...
        // Setup document click listener for click-outside-to-close
        this.doc_.addEventListener('click', this.onDocumentClick_);
    }
    ...
    onPadButtonClick_() {
        this.foldable_.set('expanded', !this.foldable_.get('expanded'));
        if (this.foldable_.get('expanded')) {
            // Focus first slider when popup opens
            if (this.sliderControllers_[0] && this.sliderControllers_[0].view.textElement) {
                this.sliderControllers_[0].view.textElement.focus();
            }
        }
    }
    ...
    onDocumentClick_(e) {
        if (!this.popC_ || !this.foldable_.get('expanded')) {
            return;
        }
        const elem = this.view.element;
...
const Point3dInputPlugin = createPlugin({
    ...
    controller: (args) => {
        ...
        return new Point3dController(args.document, {
            axes: value.rawValue.getComponents().map((comp, i) => {
                var _a;
                return createPointAxis({
                    constraint: c.components[i],
...
// THIS SECTION GENERATED BY Claude - Point4dView with button (reusing Point2d CSS classes)
class Point4dView {
    constructor(doc, config) {
        // Create main container
        this.element = doc.createElement('div');
        ...
        // Create header with button and text inputs
        const headElem = doc.createElement('div');
        headElem.classList.add(cn$p2d_reuse('h'));
        this.element.appendChild(headElem);
        
        // Create button with 4D icon
        const buttonElem = doc.createElement('button');
        buttonElem.classList.add(cn$p2d_reuse('b')); // Use Point2d button CSS
        buttonElem.appendChild(createSvgIconElement(doc, 'p4dpad'));
        config.viewProps.bindDisabled(buttonElem);
        headElem.appendChild(buttonElem);
        this.buttonElement = buttonElem;
        
        // Create text container for X,Y,Z,W inputs
        const textElem = doc.createElement('div');
        textElem.classList.add(cn$p2d_reuse('t'));
        headElem.appendChild(textElem);
        this.textElement = textElem;
    }
}

// THIS SECTION GENERATED BY Claude - Point4dController with popup functionality
class Point4dController {
    constructor(doc, config) {
        this.onPopupChildBlur_ = this.onPopupChildBlur_.bind(this);
        this.onPadButtonBlur_ = this.onPadButtonBlur_.bind(this);
        this.onPadButtonClick_ = this.onPadButtonClick_.bind(this);
        ...
        // Create 4 slider controls using proper Tweakpane controllers
        this.sliderControllers_ = [];
        ...
        labels.forEach((label, index) => {
            ...
            componentValue.emitter.on('change', () => {
                ...
                const newComponents = [...this.value.rawValue.getComponents()];
                newComponents[index] = componentValue.rawValue;
                const newValue = new Point4d(newComponents[0], newComponents[1], newComponents[2], newComponents[3]);
                this.value.rawValue = newValue;
                this.updatingFromSlider_ = false;
            });
        ...
        // Create the text controller for X,Y,Z,W inputs
        this.textC_ = new PointNdTextController(doc, {
            assembly: Point4dAssembly,
            axes: config.axes,
            parser: config.parser,
    ...
    onPadButtonClick_() {
        ...
        if (this.foldable_.get('expanded')) {
            // Focus first slider when popup opens
            if (this.sliderControllers_[0] && this.sliderControllers_[0].view.trackElement) {
                this.sliderControllers_[0].view.trackElement.focus();
            }
        }
    }
...
const Point4dInputPlugin = createPlugin({
    ...
    controller: (args) => {
        ...
        return new Point4dController(args.document, {
            axes: value.rawValue.getComponents().map((comp, i) => {
                var _a;
                return createPointAxis({
                    constraint: c.components[i],
...
class TextBladeApi extends BladeApi {
    ...
    // THIS SECTION GENERATED BY CLAUDE
    get lines() {
        return this.controller.valueController.props.get('lines');
    }
    set lines(lines) {
        this.controller.valueController.props.set('lines', lines);
        // Update textarea rows if it's a textarea
        const inputElem = this.controller.valueController.view.inputElement;
        if (inputElem.tagName.toLowerCase() === 'textarea') {
            inputElem.rows = lines;
        }
    }
    get wordwrap() {
        return this.controller.valueController.props.get('wordwrap');
    }
    set wordwrap(wordwrap) {
        this.controller.valueController.props.set('wordwrap', wordwrap);
        // Update textarea word wrapping if it's a textarea
        const inputElem = this.controller.valueController.view.inputElement;
        if (inputElem.tagName.toLowerCase() === 'textarea') {
            if (!wordwrap) {
                inputElem.style.whiteSpace = 'pre';  // THIS SECTION GENERATED BY Claude - preserve line breaks but no word wrap
            ...
            // THIS SECTION GENERATED BY Claude - Also update any syntax highlight overlay
            const container = inputElem.parentElement;
            if (container) {
                const overlayElements = container.querySelectorAll('pre, div[style*="position"], .syntax-highlight, .highlight-overlay');
                overlayElements.forEach(overlay => {
                    if (overlay !== inputElem) {
                        overlay.style.whiteSpace = wordwrap ? 'pre-wrap' : 'pre';  // Updated to use 'pre' instead of 'nowrap'
                        overlay.style.overflowX = wordwrap ? 'hidden' : 'auto';
                        overlay.style.wordWrap = wordwrap ? 'break-word' : 'normal';
                    }
                });
            }
        }
    ...
    return {
        ...
        accept(params) {
            const result = parseRecord(params, (p) => ({
                ...
                // THIS SECTION GENERATED BY CLAUDE
                multiline: p.optional.boolean,
                lines: p.optional.number,
                wordwrap: p.optional.boolean,
                // END SECTION GENERATED BY CLAUDE
            }));
            return result ? { params: result } : null;
        },
        controller(args) {
            ...
            const ic = new TextController(args.document, {
                ...
                props: ValueMap.fromObject({
                    ...
                    // THIS SECTION GENERATED BY CLAUDE
                    multiline: args.params.multiline || false,
                    lines: args.params.lines || 3,
                    wordwrap: args.params.wordwrap !== undefined ? args.params.wordwrap : true,
                    // END SECTION GENERATED BY CLAUDE
                }),
                value: v,
                viewProps: args.viewProps,
...
// THIS SECTION GENERATED BY Claude - Built-in context menu system for Tweakpane
class TweakpaneContextMenu {
    constructor() {
        this.contextMenu = null;
        this.currentControl = null;
        this.controlData = new WeakMap();
        this.controlReplacementCallbacks = new Set(); // Track replacement callbacks
        
        // Control type definitions for complex controls - THIS SECTION GENERATED BY Claude
        this.controlTypeDefinitions = {
            // Simple numeric controls
            float: {
                editableProps: ['min', 'max', 'step', 'showSlider'],
            ...
            // Vector controls with nested structure
            vec2: {
                editableProps: ['x.min', 'x.max', 'x.step', 'y.min', 'y.max', 'y.step'],
                getOptions: (options) => options,
                setOption: (options, path, value) => this.setNestedOption(options, path, value),
                getOption: (options, path) => this.getNestedOption(options, path)
            },
            vec3: {
                editableProps: ['x.min', 'x.max', 'x.step', 'y.min', 'y.max', 'y.step', 'z.min', 'z.max', 'z.step'],
                getOptions: (options) => options,
                setOption: (options, path, value) => this.setNestedOption(options, path, value),
                getOption: (options, path) => this.getNestedOption(options, path)
            },
            vec4: {
                editableProps: ['x.min', 'x.max', 'x.step', 'y.min', 'y.max', 'y.step', 'z.min', 'z.max', 'z.step', 'w.min', 'w.max', 'w.step'],
                getOptions: (options) => options,
                setOption: (options, path, value) => this.setNestedOption(options, path, value),
                getOption: (options, path) => this.getNestedOption(options, path)
            },
    ...
    /**
     * Helper to set nested option (e.g., 'x.min' -> options.x.min)
     * THIS SECTION GENERATED BY Claude
     */
    setNestedOption(options, path, value) {
    ...
    /**
     * Helper to get nested option (e.g., 'x.min' -> options.x.min)
     * THIS SECTION GENERATED BY Claude
     */
    getNestedOption(options, path) {
        const parts = path.split('.');
        let obj = options;
    ...
    /**
     * Detect control type from options structure
     * THIS SECTION GENERATED BY Claude
     */
    detectControlType(options) {
        // Check for vector controls (nested x,y,z,w properties)
        if (options && typeof options === 'object') {
            const keys = Object.keys(options);
            if (keys.includes('x') && keys.includes('y')) {
                if (keys.includes('w')) return 'vec4';
                if (keys.includes('z')) return 'vec3';
                return 'vec2';
            }
        }
        
        // Check for text controls
        if ('multiline' in options || 'lines' in options || 'wordwrap' in options) return 'text';
        
        // Check for numeric controls - THIS SECTION GENERATED BY Claude
        // Use step to determine int vs float, but default to float for safety
        if ('min' in options || 'max' in options || 'step' in options) {
            // Only treat as int if step is explicitly an integer AND > 0
            if ('step' in options && Number.isInteger(options.step) && options.step >= 1) {
                return 'int';
            }
            return 'float';
        }
    ...
    init() {
        ...
        this.contextMenu.className = 'tweakpane-context-menu';
    ...
    enableForControl(control, options = {}, recreateInfo = null) {
        this.controlData.set(control, {
        ...
        element.addEventListener('contextmenu', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.showForControl(control, e.clientX, e.clientY);
        });
    }
    
    showForControl(control, x, y) {
        this.currentControl = control;
        const data = this.controlData.get(control);
        ...
        title.textContent = 'Control Properties';
        title.style.cssText = `
            font-weight: bold;
            margin-bottom: 8px;
            padding-bottom: 4px;
        ...
        this.addPropertyEditors(data.currentOptions);
        this.show(x, y);
    }
    
    addPropertyEditors(options) {
        // Detect the control type and get its definition - THIS SECTION GENERATED BY Claude
        const controlType = this.detectControlType(options);
        const typeDef = this.controlTypeDefinitions[controlType];
        
        if (typeDef) {
            // Use sophisticated type-based property editing
            typeDef.editableProps.forEach(propPath => {
                let value;
                
                // Handle special case for showSlider - check current control state - THIS SECTION GENERATED BY Claude
                if (propPath === 'showSlider') {
                    const control = this.currentControl;
                    // Check if control currently has slider visible (by checking CSS class)
                    const sldTxtContainer = control.element.querySelector('.tp-sldtxtv') || control.element.closest('.tp-sldtxtv');
                    value = sldTxtContainer && !sldTxtContainer.classList.contains('tp-no-slider');
                } else {
                    value = typeDef.getOption ? 
                        typeDef.getOption(options, propPath) : 
                        this.getNestedOption(options, propPath);
                }
                
                // Show property even if not set, so user can set it
                const displayValue = value !== undefined ? value : '';
                // Determine input type - THIS SECTION GENERATED BY Claude
                let inputType = 'number'; // default
                if (propPath === 'showSlider' || propPath === 'wordwrap') {
                    inputType = 'checkbox';
                } else if (propPath === 'lines') {
                    inputType = 'number';
                }
                this.addPropertyEditor(propPath, displayValue, inputType);
            });
        } else {
            // Fallback to simple property editing for unknown types
            this.addSimplePropertyEditors(options);
        }
        
        // Add delete option if available - THIS SECTION GENERATED BY Claude
        const data = this.controlData.get(this.currentControl);
        if (data && data.recreateInfo && data.recreateInfo.deleteProperty) {
            ...
            deleteBtn.onclick = () => {
                this.hide();
                data.recreateInfo.deleteProperty();
            };
        ...
        this.contextMenu.appendChild(applyBtn);
    }
    
    /**
     * Fallback simple property editor for unknown control types
     * THIS SECTION GENERATED BY Claude
     */
    addSimplePropertyEditors(options) {
        // Get current values from the actual control instead of stored options
        const control = this.currentControl;
        
        // Add Show Slider checkbox for numeric controls - THIS SECTION GENERATED BY Claude
        if (options.view !== 'text' && control.constructor.name !== 'TextBladeApi') {
            // Check if control currently has slider visible (by checking CSS class)
            const sldTxtContainer = control.element.querySelector('.tp-sldtxtv') || control.element.closest('.tp-sldtxtv');
            const hasSlider = !sldTxtContainer || !sldTxtContainer.classList.contains('tp-no-slider');
            this.addPropertyEditor('showSlider', hasSlider, 'checkbox');
        }
        
        // Add property editors for numeric controls
        if (control.constructor.name !== 'TextBladeApi') {
            ['min', 'max', 'step'].forEach(prop => {
                if (control[prop] !== undefined) {
                    this.addPropertyEditor(prop, control[prop], 'number');
                } else if (prop in options) {
                    this.addPropertyEditor(prop, options[prop], 'number');
                } else {
                    // THIS SECTION GENERATED BY Claude - Show step even if not set, so user can add it
                    this.addPropertyEditor(prop, '', 'number');
                }
            });
        }
        
        // Add property editors for text controls - THIS SECTION GENERATED BY Claude
        if (control.constructor.name === 'TextBladeApi') {
            // Get current values from the control's getters
            this.addPropertyEditor('wordwrap', control.wordwrap, 'checkbox');
            this.addPropertyEditor('lines', control.lines, 'number');
        }
    }
    
    addPropertyEditor(propName, value, type) {
        const row = document.createElement('div');
        row.style.cssText = `display: flex; align-items: center; margin: 4px 0; gap: 4px;`;
        
        const label = document.createElement('label');
        // Format nested property names (e.g., 'x.min' -> 'X Min') - THIS SECTION GENERATED BY Claude
        const displayName = propName.split('.').map(part => 
            part.charAt(0).toUpperCase() + part.slice(1)
        ).join(' ');
        label.textContent = displayName;
        label.style.cssText = `min-width: 60px; text-transform: capitalize; font-size: 11px;`;
        
        const input = document.createElement('input');
        input.type = type;
        
        // Set value or checked property based on input type - THIS SECTION GENERATED BY Claude
        if (type === 'checkbox') {
        ...
        input.style.cssText = `
            flex: 1; padding: 2px 4px; background: #333; border: 1px solid #555;
            color: #ccc; border-radius: 2px; font-size: 11px;
        `;
        input.dataset.property = propName;
        
        // Add event listener for immediate updates - THIS SECTION GENERATED BY Claude
        const updateHandler = () => {
            this.updatePropertyImmediately(propName, input);
        };
        ...
        // Add clear button for min/max/step properties (including nested) - THIS SECTION GENERATED BY Claude
        if (propName.includes('min') || propName.includes('max') || propName.includes('step')) {
            const clearBtn = document.createElement('button');
            clearBtn.textContent = '×';
            clearBtn.title = 'Clear ' + displayName;
            clearBtn.style.cssText = `
                padding: 2px 6px; background: #666; color: #ccc; border: none;
                border-radius: 2px; cursor: pointer; font-size: 10px; line-height: 1;
            `;
            clearBtn.onclick = () => {
                input.value = '';
                input.dataset.cleared = 'true';
                // Immediately apply the clear for step to remove constraint
                if (propName.includes('step')) {
                    this.updatePropertyImmediately(propName, input);
                }
            };
    ...
    updatePropertyImmediately(propName, inputElement) {
        if (!this.currentControl) return;
        
        const data = this.controlData.get(this.currentControl);
        if (!data) return;
        
        // Detect control type for nested property handling - THIS SECTION GENERATED BY Claude
        const controlType = this.detectControlType(data.currentOptions);
        const typeDef = this.controlTypeDefinitions[controlType];
        ...
        if (inputElement.type === 'checkbox') {
            ...
            value = inputElement.value === '' ? undefined : parseFloat(inputElement.value) || 0;
        } else {
            value = inputElement.value === '' ? undefined : inputElement.value;
        }
        
        // Update stored options using type definition or fallback - THIS SECTION GENERATED BY Claude
        if (typeDef && typeDef.setOption) {
            typeDef.setOption(data.currentOptions, propName, value);
        } else {
            // Simple property update for basic controls
            if (value === undefined) {
                delete data.currentOptions[propName];
            } else {
                data.currentOptions[propName] = value;
            }
        }
        
        // For immediate updates, handle special cases - THIS SECTION GENERATED BY Claude
        if (propName === 'showSlider') {
            // Handle showSlider by recreating the control with appropriate options
            const recreateInfo = data.recreateInfo;
            if (recreateInfo && recreateInfo.target && recreateInfo.property && recreateInfo.folder) {
                // Update the current options first
                const newOptions = { ...data.currentOptions };
                
                if (value) {
                    // Enable slider: ensure min/max are set
                    if (newOptions.min === undefined) newOptions.min = 0;
                    if (newOptions.max === undefined) newOptions.max = 100;
                    // THIS SECTION GENERATED BY Claude - Don't default step for sliders to allow float input
                    // if (newOptions.step === undefined) newOptions.step = 1;
                } else {
                    // Disable slider: remove min/max to force text input
                ...
                // Create new control with updated options
                const newControl = recreateInfo.folder.addBinding(
                    recreateInfo.target, 
                    recreateInfo.property, 
                    newOptions
                );
                
                // Mark the new control to skip auto-context menu
                newControl._skipAutoContextMenu = true;
                
                // Transfer context menu data to new control
                this.controlData.delete(this.currentControl);
                this.currentControl = newControl;
                
                // Re-enable context menu for new control
                setTimeout(() => {
                    this.enableForControl(newControl, newOptions, recreateInfo);
                }, 0);
            }
        } else if (!propName.includes('.')) {
            // Handle other simple properties that don't require control recreation
            const updateObj = {};
            updateObj[propName] = value;
            this.updateControl(this.currentControl, updateObj);
        }
    }
    ...
    applyChanges() {
        ...
        // Detect control type and get type definition - THIS SECTION GENERATED BY Claude
        const controlType = this.detectControlType(data.currentOptions);
        const typeDef = this.controlTypeDefinitions[controlType];
        
        const inputs = this.contextMenu.querySelectorAll('input');
        const newOptions = JSON.parse(JSON.stringify(data.currentOptions));
        
        inputs.forEach(input => {
            const propPath = input.dataset.property;
            let value;
            
            // Handle cleared properties
            if (input.dataset.cleared === 'true' || input.value === '') {
                value = undefined;
            } else {
            ...
            // Set the value using control type definition or fallback - THIS SECTION GENERATED BY Claude
            if (typeDef && typeDef.setOption) {
                typeDef.setOption(newOptions, propPath, value);
            } else {
                // Simple property setting for basic controls
                if (value === undefined) {
                    delete newOptions[propPath];
                } else {
                    newOptions[propPath] = value;
                }
            }
        });
        
        data.currentOptions = newOptions;
        this.updateControl(this.currentControl, newOptions);
        this.hide();
    }
      updateControl(control, options) {
        const data = this.controlData.get(control);
        if (!data) return;

        // THIS SECTION GENERATED BY Claude - Modify existing control properties instead of recreating
        
        // Handle text controls (TextBladeApi) - THIS SECTION GENERATED BY Claude
        if (control.constructor.name === 'TextBladeApi') {
            if (options.wordwrap !== undefined) {
                control.wordwrap = options.wordwrap;
            }
            if (options.lines !== undefined) {
                control.lines = options.lines;
            }
        } else {
            // Handle numeric controls
            // Update min/max on slider controls
            ...
            if (control.step !== undefined && options.step !== undefined) {
                control.step = options.step;
            }
            
            // Handle showSlider flag by toggling CSS class - THIS SECTION GENERATED BY Claude
            if (options.showSlider !== undefined) {
                const controlContainer = control.element.querySelector('.tp-sldtxtv') || control.element.closest('.tp-sldtxtv');
                if (controlContainer) {
                    if (options.showSlider) {
                        controlContainer.classList.remove('tp-no-slider');
                    } else {
                        controlContainer.classList.add('tp-no-slider');
                    }
                }
            }
        }
        ...
        data.currentOptions = { ...data.currentOptions, ...options };
    }
...
// Global context menu instance - THIS SECTION GENERATED BY Claude
const globalContextMenu = new TweakpaneContextMenu();

// Patch FolderApi to automatically enable context menus - THIS SECTION GENERATED BY Claude
const originalFolderAddBinding = FolderApi.prototype.addBinding;
const originalFolderAddBlade = FolderApi.prototype.addBlade;

FolderApi.prototype.addBinding = function(target, property, options = {}) {
    // Handle showSlider flag - THIS SECTION GENERATED BY Claude
    const processedOptions = { ...options };
    if (processedOptions.showSlider !== undefined) {
        if (processedOptions.showSlider) {
            // Ensure we have min/max for slider behavior
            if (processedOptions.min === undefined) processedOptions.min = 0;
            if (processedOptions.max === undefined) processedOptions.max = 100;
            // THIS SECTION GENERATED BY Claude - Don't default step for sliders to allow float input
            // if (processedOptions.step === undefined) processedOptions.step = 1;
        } else {
            // Remove min/max to force text input
            delete processedOptions.min;
            delete processedOptions.max;
        }
        // Remove the showSlider flag as Tweakpane doesn't understand it
        delete processedOptions.showSlider;
    }
    
    const control = originalFolderAddBinding.call(this, target, property, processedOptions);
    
    // Auto-enable context menu for input controls
    if (control && control.element) {
        // Wait a tick for DOM to be ready
        setTimeout(() => {
            if (control.element.querySelector('input, textarea')) {
                // Skip auto-patching if control is marked as handled by expandable folder - THIS SECTION GENERATED BY Claude
                if (control._skipAutoContextMenu) return;
                
                // Auto-detect vector controls by inspecting the bound value - THIS SECTION GENERATED BY Claude
                let enhancedOptions = { ...options };
                const value = target[property];
                if (value && typeof value === 'object' && 'x' in value && 'y' in value) {
                    // This is a vector control - add component constraints if not already present
                    if (!enhancedOptions.x) {
                        enhancedOptions.x = { min: -100, max: 100, step: 0.1 };
                        enhancedOptions.y = { min: -100, max: 100, step: 0.1 };
                        if ('z' in value) {
                            enhancedOptions.z = { min: -100, max: 100, step: 0.1 };
                        }
                        if ('w' in value) {
                            enhancedOptions.w = { min: -100, max: 100, step: 0.1 };
                        }
                    }
                }
                
                const recreateInfo = { target, property, folder: this };
                globalContextMenu.enableForControl(control, enhancedOptions, recreateInfo);
            }
        }, 0);
    }
...
FolderApi.prototype.addBlade = function(params = {}) {
    const control = originalFolderAddBlade.call(this, params);
    
    // Auto-enable context menu for text blades
    if (control && control.element && params.view === 'text') {
        setTimeout(() => {
            if (control.element.querySelector('input, textarea')) {
                const recreateInfo = { params, folder: this };
                globalContextMenu.enableForControl(control, params, recreateInfo);
            }
        }, 0);
    }
...
// THIS SECTION GENERATED BY Claude - Expandable Folder Extension
/**
 * Add expandable folder functionality to Pane and FolderApi
 * Creates a folder with a "+" button that can dynamically add properties
 * @param {Object} config - Configuration object
 * @param {string} config.title - Folder title
 * @param {Object} config.target - Target object to bind properties to
 * @param {Function} config.onChange - Callback when properties change (name, value, type)
 * @param {Object} config.configTypes - Available types with default values
 *   Example: { float: 0.0, vector: {x:1, y:2}, vec3d: {x:0, y:0, z:0} }
 * @returns {Object} Folder with addProperty method
 */
function addExpandableFolder(config) {
    const {
        title = 'Properties',
        target = {},
        onChange = () => {},
        configTypes = { float: 0.0, bool: false, color: {r:255, g:255, b:255} }
    } = config;

    // Create the folder
    const folder = this.addFolder({ title, expanded: true });
    const bindings = new Map();

    /**
     * Track which properties existed when the folder was created
     */
    const originalProperties = new Set(Object.keys(target));
    
    function isExistingProperty(name) {
        return originalProperties.has(name);
    }

    /**
     * Determine the type name from a value (best guess for existing properties)
     */
    ...
    /**
     * Setup context menu for a binding
     */
    function setupContextMenuForBinding(binding, name, isDynamic) {
        // Clear any existing context menu data to avoid conflicts
        if (globalContextMenu.controlData.has(binding)) {
            globalContextMenu.controlData.delete(binding);
        }
        
        setTimeout(() => {
            const value = target[name];
            
            // Determine appropriate options for context menu based on type
            ...
            if (typeof value === 'number') {
                ...
                // Vector types (vec2, vec3, vec4) - add component constraints
                contextMenuOptions = {
                    x: { min: -100, max: 100, step: 0.1 },
                    y: { min: -100, max: 100, step: 0.1 }
                };
                if ('z' in value) {
                    contextMenuOptions.z = { min: -100, max: 100, step: 0.1 };
                }
                if ('w' in value) {
                    contextMenuOptions.w = { min: -100, max: 100, step: 0.1 };
                }
            }
            ...
            const recreateInfo = {
                ...
                property: name,
                folder: folder,
                callback: () => onChange(name, target[name], getTypeNameFromValue(target[name])),
                // THIS SECTION GENERATED BY Claude - Enable deletion for all properties
                deleteProperty: () => {
                    const confirmMessage = isDynamic 
                        ? `Delete dynamic property "${name}"?`
                        : `Delete existing property "${name}"? This will remove it from the target object.`;
            ...
            // Use existing context menu system with proper options
            globalContextMenu.enableForControl(binding, contextMenuOptions, recreateInfo);
        }, 0);
    }

    /**
     * Create a binding for an existing property in the target object
     */
    function createBindingForProperty(name) {
        if (!target.hasOwnProperty(name) || bindings.has(name)) {
            return false;
        }
        ...
        // Create a wrapper to mark the control before auto-patch runs
        const originalAddBinding = folder.addBinding;
        folder.addBinding = function(target, property, options) {
            const control = originalAddBinding.call(this, target, property, options);
            control._skipAutoContextMenu = true; // Mark before auto-patch setTimeout
            return control;
        };
        
        if (typeof value === 'number') {
            // THIS SECTION GENERATED BY Claude - Don't set step by default for floats
            if (Number.isInteger(value)) {
                binding = folder.addBinding(target, name, { step: 1 });
            } else {
                // For floats, create without step to allow free entry
                binding = folder.addBinding(target, name);
            }
        } else if (typeof value === 'boolean') {
            binding = folder.addBinding(target, name);
        } else if (value && typeof value === 'object') {
            if ('r' in value && 'g' in value && 'b' in value) {
                binding = folder.addBinding(target, name, { view: 'color' });
            } else if ('x' in value && 'y' in value) {
                if ('z' in value) {
                    binding = folder.addBinding(target, name, { view: 'point3d' });
                } else {
                    binding = folder.addBinding(target, name, { view: 'point2d' });
                }
            } else {
                binding = folder.addBinding(target, name);
            }
        ...
        // Restore original addBinding
        folder.addBinding = originalAddBinding;
        
        // Add change listener
        binding.on('change', () => onChange(name, target[name], getTypeNameFromValue(target[name])));
        
        // Add context menu - determine if this is a dynamically added property or existing one
        const isDynamic = !isExistingProperty(name);
        setupContextMenuForBinding(binding, name, isDynamic);
        
        bindings.set(name, binding);
        return true;
    }

    // Auto-detect and add existing properties in the target object
    for (const key in target) {
        if (target.hasOwnProperty(key)) {
            createBindingForProperty(key);
        }
    }

    // Add "+" button to folder header
    setTimeout(() => {
        const folderHeader = folder.element.querySelector('.tp-fldv_b');
        ...
        addButton.onmouseenter = () => addButton.style.backgroundColor = 'var(--btn-bg-h)';
        addButton.onmouseleave = () => addButton.style.backgroundColor = 'var(--btn-bg)';
        addButton.onclick = (e) => {
            e.stopPropagation();
            showAddDialog();
        };

        folderHeader.style.position = 'relative';
     ...
     */
    function addProperty(name, typeName) {
        if (!name || configTypes[typeName] === undefined || target.hasOwnProperty(name)) {
            return false;
        }
        
        // Set default value
        target[name] = JSON.parse(JSON.stringify(configTypes[typeName]));
        
        // Create binding using the shared logic
        return createBindingForProperty(name);
    }
    ...
    function removeProperty(name) {
        ...
        if (binding) {
            ...
            onChange(name, undefined, 'deleted');
        }
    }

    /**
     * Show add property dialog as temporary controls in folder
     * THIS SECTION GENERATED BY Claude
     */
    function showAddDialog() {
        // Remove existing add controls if any
        const existing = folder.element.querySelector('.tp-add-property-controls');
        if (existing) return; // Don't add multiple

        // Create temporary container for add controls
        const addControls = document.createElement('div');
        addControls.className = 'tp-add-property-controls';
        ...
        nameRow.style.cssText = `display: flex; align-items: center; margin-bottom: 4px; gap: 8px;`;
        nameRow.innerHTML = `
            <label style="min-width: 40px; font-size: 11px; color: #999;">Name:</label>
            <input type="text" placeholder="Property name" style="
                flex: 1; padding: 4px 6px; border: 1px solid #444;
                border-radius: 2px; background: #1a1a1a; color: #ccc;
                font-size: 11px;
            ">
        `;
        ...
        typeRow.style.cssText = `display: flex; align-items: center; margin-bottom: 6px; gap: 8px;`;
        const typeOptions = Object.keys(configTypes).map(type => 
            `<option value="${type}">${type}</option>`
        ).join('');
        typeRow.innerHTML = `
            <label style="min-width: 40px; font-size: 11px; color: #999;">Type:</label>
            <select style="
                flex: 1; padding: 4px 6px; border: 1px solid #444;
                border-radius: 2px; background: #1a1a1a; color: #ccc;
                font-size: 11px;
        ...
        buttonRow.style.cssText = `display: flex; gap: 4px; justify-content: flex-end;`;
        buttonRow.innerHTML = `
            <button class="cancel-btn" style="
                padding: 4px 8px; border: 1px solid #444; background: #333;
                color: #ccc; border-radius: 2px; cursor: pointer; font-size: 10px;
            ">Cancel</button>
            <button class="add-btn" style="
                padding: 4px 8px; border: none; background: #007acc;
                color: white; border-radius: 2px; cursor: pointer; font-size: 10px;
            ">Add</button>
        `;

        addControls.appendChild(nameRow);
        addControls.appendChild(typeRow);
        addControls.appendChild(buttonRow);

        // Insert at top of folder content
        const folderContent = folder.element.querySelector('.tp-fldv_c');
        if (folderContent && folderContent.firstChild) {
            folderContent.insertBefore(addControls, folderContent.firstChild);
        } else if (folderContent) {
            folderContent.appendChild(addControls);
        }

        // Get references
        const nameInput = nameRow.querySelector('input');
        const typeSelect = typeRow.querySelector('select');
        const cancelBtn = buttonRow.querySelector('.cancel-btn');
        const addBtn = buttonRow.querySelector('.add-btn');

        // Focus name input
        nameInput.focus();

        // Handle buttons
        cancelBtn.onclick = () => addControls.remove();
        addBtn.onclick = () => {
    ...
    // Return folder with extension methods
...
// Add to Pane and FolderApi prototypes
Pane.prototype.addExpandableFolder = addExpandableFolder;
FolderApi.prototype.addExpandableFolder = addExpandableFolder;
